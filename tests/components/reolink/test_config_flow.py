...
from homeassistant.components.reolink import DEVICE_UPDATE_INTERVAL, DOMAIN, const
from homeassistant.components.reolink.config_flow import DEFAULT_PROTOCOL
from homeassistant.components.reolink.const import CONF_USE_HTTPS
...
async def test_config_flow_manual_success(
    hass: HomeAssistant, reolink_connect: AsyncMock, mock_setup_entry: AsyncMock
) -> None:
...
async def test_config_flow_no_admin(
    hass: HomeAssistant, reolink_connect: AsyncMock, mock_setup_entry: AsyncMock
) -> None:
...
@pytest.mark.parametrize(
    ("exception", "errors"),
    [
        (ReolinkError("Test error"), {CONF_HOST: "cannot_connect"}),
        (ReolinkWebhookException("Test error"), {"base": "webhook_exception"}),
        (JSONDecodeError("test_error", "test", 1), {CONF_HOST: "unknown"}),
        (CredentialsInvalidError("Test error"), {CONF_HOST: "invalid_auth"}),
        (ApiError("Test error"), {CONF_HOST: "api_error"}),
    ],
)
async def test_config_flow_errors(
    hass: HomeAssistant,
    reolink_connect: AsyncMock,
    ...
    reolink_connect.get_host_data.side_effect = exception
...
async def test_dhcp_while_connected_doesnt_change(
    hass: HomeAssistant,
    reolink_connect: AsyncMock,
    mock_setup_entry: AsyncMock,
    config_entry: MockConfigEntry,
...
async def test_dhcp_not_connected(
    hass: HomeAssistant,
    reolink_connect: AsyncMock,
    mock_setup_entry: AsyncMock,
    freezer: FrozenDateTimeFactory,
    reolink_connect_class: AsyncMock,
    config_entry: MockConfigEntry,
) -> None:
    """Test dhcp discovery aborts if already configured."""
    ...
    # ensure the last_update_success is False for the device_coordinator.
